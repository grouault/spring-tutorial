...
public class OperationService extends AbstractService implements IOperationService {
	...
	public IOperationEntity select(int unUtilisateurId, int unCompteId, int uneOperationId)
		...
		try {
			compte = this.getCompteDao().findOne(Integer.valueOf(unCompteId));
		} catch (Exception e) {
		...
		try {
			resultat = this.getOperationDao().findOne(Integer.valueOf(uneOperationId));
		} catch (Exception e) {
			throw new ErreurTechniqueException(e);
		}
		if (resultat == null) {
	...
	public List<IOperationEntity> selectAll(int unUtilisateurId, int unCompteId)
		...
		try {
			resultat = this.getOperationDao().selectCriteria(unCompteId, null, null, null);
	...
	public List<IOperationEntity> faireVirement(int unUtilisateurId, int unCompteIdSrc, int unCompteIdDst,
		...
		try {
			compteSrc = this.getCompteDao().findOne(Integer.valueOf(unCompteIdSrc));
		} catch (Exception e) {
			throw new ErreurTechniqueException(e);
		}
		if (compteSrc == null) {
		...
		try {
			compteDst = this.getCompteDao().findOne(Integer.valueOf(unCompteIdDst));
		} catch (Exception e) {
			throw new ErreurTechniqueException(e);
		}
		if (compteDst == null) {
		...
		try {
			...
			this.getCompteDao().save(compteSrc);
			this.getCompteDao().save(compteDst);
